...
class MainGui(QMainWindow):
    def __init__(self):
        ...
        self.tableView.setColumnWidth(2, 100)  # Set width for release date column
    ...
    def train_knn_model(self, data):
        song_features = ['release_date','danceability', 'energy', 'key', 'loudness', 'mode',
                         'speechiness', 'acousticness', 'instrumentalness', 'liveness', 'valence', 'tempo',
                         'time_signature']
        X = data[song_features]
    ...
    def recommend_songs(self):
        ...
        if favorites['release_date'].dtype == 'object':  # apply only if it's not already in datetime format
        ...
        df_features = df[song_features]

        scaler = StandardScaler()
        df_features_scaled = pd.DataFrame(scaler.fit_transform(df_features), columns=song_features)

        model_knn = NearestNeighbors(metric='cosine', algorithm='brute', n_neighbors=100, n_jobs=-1)
        model_knn.fit(df_features_scaled)

        distances, indices = model_knn.kneighbors(favorites[song_features])

        recommended_songs = df.iloc[indices.flatten()]
        recommended_songs = recommended_songs.loc[
            ~recommended_songs['id'].isin(favorites['id'])]  # Exclude songs already in favorites

        if recommended_songs.empty:
            self.output_label.setPlainText("No songs to recommend.")
        else:
            model = self.to_table_model(recommended_songs)
            self.tableView.setModel(model)
